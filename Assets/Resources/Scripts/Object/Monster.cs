...
public class Monster : MonoBehaviour, IHealth
{
        ...
        if (!effects.Any(x => x.HasStatusEffect("기절")) && target.CurrentHp() > 0)
        {
            switch (monsterBattleStat.DecideAction())
            {
                case MonsterActionType.Attack:
                    yield return new WaitForSeconds(0.65f);    // 살짝 지연시켜서 몬스터들의 공격이 빠르게 처리되지 않도록 함
                    animator.Play("Attack");
                    yield return new WaitForSeconds(animator.GetCurrentAnimatorStateInfo(0).length);
                    // 게임을 재시작하면 비활성화되기 때문에 대미지 처리를 하지 않도록 예외처리
                    if (gameObject.activeInHierarchy)
                        EventBus<int>.Publish(GameEventType.ENEMYATTACK, runtimeStat.currentAtk);
                    break;
    ...
    private void ExecuteDamageProcess(int damage)
    {
    ...
    public void TakeDamage(int damage)
    {
        ...
        ExecuteDamageProcess(damage);
    }

    // 카드가 상태이상을 부여할 때 IHealth의 AddStatusEffect 함수를 호출함
    ...
    public void CheckStatusEffect()
    {
        ...
        {
            if (effects[i].HasStatusEffect("독"))
                ExecuteDamageProcess(effects[i].Damage);
